...
module.exports = function (key, secret, redirect) {
  ...
  CLIENT.apiCall = function (method, path, params, callback) {
    ...
    // CI EDIT !!!
    // Not sure why this is here. Prob something to do with security
    // delete params.token;
    // END CI EDIT
    ...
    if (method.toUpperCase() === 'GET') {
      ...
      // IT COCKS UP SUBSEQUENT REQUESTS
      // THE FIX IS TO COMMENT THIS BLOCK OUT AND
      // IN THE REQUEST PATH TO APPEND AN & IF THE PATH CONTAINS A ?
      // if (path.match(hasParameters)) {
      //     paramAppender = "&";
      // }
      // console.log(_rest_base + path + paramAppender + querystring.stringify(params));
